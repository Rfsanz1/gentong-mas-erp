...
export default function AgedPayablePage() {
  ...
  const apQuery = useQuery<APAgingReport>({
    queryKey: ['ap-aging', asOf],
    queryFn: () => apiGet<APAgingReport>('/api/finance/ap-aging', { params: { asOf } }),
    retry: false,
  });

  const data = apQuery.data;
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-slate-900">Hutang — AP Aging</h1>
          <p className="text-sm text-slate-500 mt-1">Analisis umur hutang kepada supplier.</p>
        </div>
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        {apQuery.isLoading ? <LoadingState message="Memuat AP aging..." /> :
          !data || data.items.length === 0 ? <EmptyState message="Tidak ada hutang outstanding." /> : (
                ...
                <tbody className="divide-y divide-slate-50">
                  {data.items.sort((a, b) => b.total - a.total).map((item) => (
                    <tr key={item.supplierId} className="hover:bg-slate-50">
                      <td className="py-2.5 font-medium text-slate-900">{item.supplierName}</td>
                      {COLUMNS.map((c) => (
